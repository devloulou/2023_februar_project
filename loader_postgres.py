...
"""
    ha postgres == movies -> nem kell csinálni semmit
    ha a postgres tartalmaz olyan elemet, 
    ami nincs benne a movies-ban -> törlöm ezt az adatot a postgresből és posters folderekből

    ha a movies -ban van olyan adat, ami nincs a postgres-ben -> letölteni a metaadatot
"""
...
def download_metadata():    
    ...
    movie_lower = sorted([item.lower() for item in movies])
    meta_data = sorted([item[0].lower() for item in sql.run_query(select_meta, True)])

    if movie_lower == meta_data:
        return

    for item in meta_data:
        if item not in movie_lower:
    ...
    for movie in [item for item in movie_lower if item not in meta_data]:
        title = movie
